...
function updateFromMetrics(metrics, organization) {
    ...
    metrics.forEach(m => {
        ...
        devices.set(m.ip, {
            ...
            ip: m.ip,
            status: m.status,
            latency: m.latency ?? existing.latency ?? 0,
            packetLoss: m.packetLoss ?? existing.packetLoss ?? 0,
            cpuUsage: m.cpuUsage ?? existing.cpuUsage ?? 0,
            memoryUsage: m.memoryUsage ?? existing.memoryUsage ?? 0,
            trafficIn: m.trafficIn ?? existing.trafficIn ?? 0,
            trafficOut: m.trafficOut ?? existing.trafficOut ?? 0,
            uptime: m.uptime ?? existing.uptime ?? 0,
            alive: m.alive,
